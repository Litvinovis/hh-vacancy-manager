...
public class SearchRepository {
    ...
    /** Pipeline run order: higher run_priority first (see PipelineScheduler.runPipeline), tiebroken as before. */
    public List<SearchConfig> findAllEnabled() {
        return jdbc.query("SELECT * FROM searches WHERE enabled = 1 ORDER BY run_priority DESC, user_id, id", rowMapper);
    }
